...
func TestRequestReturns200(t *testing.T) {
	...
	req, reqErr := loginradius.CreateRequest("GET", ts.URL, "")
	if reqErr != nil {
		fmt.Println(reqErr)
		t.Errorf("Shouldn't be error.")
	}
	data := new(Success)
	err := loginradius.RunRequest(req, data)
	if err != nil {
		fmt.Println(err)
		t.Errorf("Shouldn't be error.")
	}
...
func setupEmailVerificationAccount(t *testing.T) (string, string, string, func(t *testing.T)) {
	...
	token, err3 := loginradius.PostManageEmailVerificationToken(tokenGen)
	if err3 != nil {
...
func getPasswordResetToken(email string) string {
	...
	passToken, err := loginradius.PostManageForgotPasswordToken(resetEmail)
	if err != nil {
		return ""
	}
